 ...
 * unknown unit cannot be constructed, so it can never reach the DAO. Cross-field rules
 * (conditional unit fields, pair integrity) are enforced by [HistoryWriteValidator].
 *
 * Separate command types give structural [event_detail_integrity]: a dose command can only produce
 * a dose detail, a parameter command only a parameter detail (never UI-trusted).
 */
...
/** Calibration payload required when a measurement unit needs it ([UnitCode.requiresCalibration]). */
data class Calibration(
    val scoopDefinitionId: String? = null,            // for MANUFACTURER_SCOOP
    val calibratedVolume: Quantity? = null,           // for USER_CALIBRATED_SPOON (volume + its unit)
)

/** Rounding provenance for a computed amount (both or neither — pair integrity). */
data class RoundingInfo(val modeCode: String, val scale: Int)

/** Record an actually-administered dose (never auto-created from a displayed calculation). */
data class RecordDoseCommand(
    ...
    val calculated: Quantity? = null,                 // pairs by construction (value+unit together)
    val concentration: Quantity? = null,
    val route: AdministrationRoute? = null,
    val rounding: RoundingInfo? = null,
    val administeredCalibration: Calibration? = null,
...
/** Record a measured water-parameter reading. */
...
data class CorrectionCommand(
    ...
    val reason: String,
    val replacement: RecordDoseCommand? = null,       // optional corrected dose payload
    val notes: String? = null,
)
