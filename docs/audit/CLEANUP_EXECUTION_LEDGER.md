...
| `Feature_Update/` | Orphaned salt-mix JS drop | ADR/web comments only; no build, CI, script, import, release, or submodule use | delete | Salt-mix factors are already in active Kotlin and TS engine code. | `SaltMixCalculations.kt`; `web/src/engine/salt-mix.ts` | `testDebugUnitTest`, `lintDebug`, `assembleDebug`, `assembleRelease`, `web npm test` | pending |
