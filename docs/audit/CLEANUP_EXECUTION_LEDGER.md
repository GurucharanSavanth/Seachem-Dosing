...
| `activity_main.xml` + ConstraintLayout dependency | Three-row app shell: app bar, nav host, bottom nav | Layout and Gradle only; no barriers, guidelines, transitions, overlapping constraints, or other ConstraintLayout XML | replace/delete | A vertical LinearLayout with weighted FragmentContainerView preserves order, IDs, insets, nav host sizing, and visibility behavior with less dependency surface. | Native LinearLayout | `testDebugUnitTest`, `lintDebug`, `assembleDebug`, `assembleRelease`, `connectedDebugAndroidTest` | b1c9c1c |
| `CalcResult.map`, `CalcResult.fold`, `CalcResult.isSuccess` | Convenience API on sealed result | Definitions plus `CalcResultTest`; no production, Java, reflection, serialization, docs API, or binary-compat requirement | delete | Callers already use exhaustive `when`; helpers widened API surface without current value. | `failureOrNull` retained for engine guard short-circuiting | `testDebugUnitTest`, `lintDebug`, `assembleDebug`, `assembleRelease`, `connectedDebugAndroidTest` | e44a0d5 |
