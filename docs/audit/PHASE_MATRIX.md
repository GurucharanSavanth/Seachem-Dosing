...
| 2-3. Result model + engines | Sealed result model, validation/unit engines, BigDecimal-safe dosing wrappers. | Partial: `CalcResult`, `ValidationEngine`, `UnitConversionEngine`, `FertilizerChemistryEngine`, recommendation and medication safety/search exist. Obsolete `DosingResult` staging was removed; no `DosingCalculationEngine` wrapper. | JVM tests cover result, validation, unit conversion, fertilizer, recommendation, medication safety/search. Latest generated JVM report: 138 tests, 0 fail/error/skip. | SPEC still marks T6 incomplete. | Implement T6 only if product flow needs it; current app still uses `MainViewModel` + `SeachemCalculations` directly. |
...
| 10. Release readiness | Release build, signing boundary, SBOM/notices, final clean tree. | Partial: release APK is generated by `assembleRelease`; signing is conditional; Apache-2.0 license and notices exist. | Cleanup preflight reran `assembleRelease`; CI does not run release build. | Release docs stale/overstated; generated release binaries should not be source-tracked. | Publish release outputs as CI/release artifacts; final tree must exclude generated binaries. |
